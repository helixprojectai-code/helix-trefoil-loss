...
def test_trace_invariant_penalty():
    ...
    total_loss_perfect = criterion(
        base_loss, current_gamma=1 / 3, weight_tensors=[perfect_weight]
    )
    ...
    total_loss_drift = criterion(
        base_loss, current_gamma=1 / 3, weight_tensors=[drifting_weight]
    )
