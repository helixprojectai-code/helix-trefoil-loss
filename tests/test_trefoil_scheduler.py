...
def test_scheduler_initialization():
    scheduler = TrefoilScheduler(initial_gamma=1.0, target_gamma=1/3, total_epochs=100)
    assert scheduler.get_gamma() == 1.0
    assert scheduler.target_gamma == 1/3

def test_linear_annealing():
    scheduler = TrefoilScheduler(initial_gamma=1.0, target_gamma=1/3, total_epochs=10, anneal_strategy='linear')
...
def test_cosine_annealing():
    scheduler = TrefoilScheduler(initial_gamma=1.0, target_gamma=1/3, total_epochs=100, anneal_strategy='cosine')
