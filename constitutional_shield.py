...
def constitutional_shield(max_penalty: float = 950.0, penalty_threshold_ratio: float = 100.0):
    """
    ...
    def decorator(func):
        ...
        def wrapper(self, base_loss, current_gamma, weight_tensors=None):
            ...
            if isinstance(current_gamma, (int, float)):
                current_gamma = torch.tensor(current_gamma, device=device, dtype=torch.float32)
            else:
