...
def simulate_training(noise_level: float = 0.5, use_trefoil: bool = False, epochs: int = 100) -> List[Tuple[int, float, float]]:
    gamma_attractor = 1/3
...
def plot_arnold_tongue(standard_res: List[Tuple], trefoil_res: List[Tuple]):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(epochs, std_acc, 'r--', alpha=0.7, label='Standard Adam (Unratified Space)')
    ...
    plt.plot(epochs, trf_gamma, 'b-', linewidth=2, label='Governed $\\gamma$ (Rubber Pants Active)')
    plt.axhline(y=1/3, color='g', linestyle=':', linewidth=2, label='Attractor ($\\gamma=1/3$)')
